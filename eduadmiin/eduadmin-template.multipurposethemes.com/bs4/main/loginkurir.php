...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['username']) && isset($_POST['password'])) {
        ...
        $stmt = $conn->prepare("SELECT id, username, password FROM kurir WHERE username = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($password == $db_password) {  // Use password_verify() for hashed passwords
                ...
                header("Location: /food-funday-master/theme-indomarket-master/kurir.php");
                exit();
            } else {
    ...
    header("Location: login_kurir.html");
    exit();
}
?>