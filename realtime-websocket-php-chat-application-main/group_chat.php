...
<html>
    ...
    <div class="input-group mb-3">
        <textarea class="form-control" id="group_chat_message" name="group_chat_message." placeholder="Type Message Here" data-parsley-maxlength="1000" data-parsley-pattern="/^[a-zA-Z0-9\s\?]+$/" required></textarea>
...
<div id="group_chat_message"></div> <!-- Display sent messages here -->
<div id="validation_error"></div> <!-- Display validation errors here -->
...
<script>
    $(document).ready(function() {
        $('#chat_form').on('submit', function(event) {
            ...
            // Validate the form using Parsley if required
            if ($(this).parsley().isValid()) {
                ...
                $.ajax({
                    url: 'save_message.php', // Replace with your server endpoint
                    method: 'POST',
                    data: { chat_message: message },
                    success: function(response) {
                        // Optionally, display the message in chat
                        $('#group_chat_message').append('<p>' + message + '</p>');
